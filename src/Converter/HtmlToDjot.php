...
class HtmlToDjot
{
    ...
    protected function processSection(DOMElement $node): string
    {
        ...
        } elseif ($sectionId !== '' && $firstHeading !== null && !$this->sectionIdIsAutoGenerated($sectionId, $firstHeading)) {
            // No round-trip annotation, but the section id differs from the slug
            // Djot derives from the heading text: it is an explicit `{#id}` that
            // must be preserved. Auto-derived ids are left implicit so the next
            // parse regenerates them identically.
            $headingAttrs = $this->getElementAttributes($firstHeading, ['id', 'data-djot-source-level']);
            $attrParts = ['#' . $sectionId];
            if ($headingAttrs !== '') {
                $attrParts[] = $headingAttrs;
            }
    ...
    /**
     * Whether a section id is the slug Djot would auto-generate from the heading
     * text, and therefore safe to leave implicit (it regenerates identically).
     *
     * When the heading text has no usable slug (empty, all-punctuation, or a
     * non-transliterable script) Djot falls back to a generated `s-N` id. Only
     * ids matching that pattern are auto in this case; any other id is an
     * explicit `{#id}` that must be preserved.
     */
    protected function sectionIdIsAutoGenerated(string $sectionId, DOMElement $heading): bool
    {
        $autoId = (new HeadingIdTracker())->normalizeId(trim($heading->textContent));
        if ($autoId !== '') {
            return $sectionId === $autoId;
        }

        return preg_match('/^s-\d+$/', $sectionId) === 1;
    }
    ...
    protected function processLink(DOMElement $node): string
    {
        ...
        // Footnote reference emitted without round-trip annotations (the default
        // DjotConverter output): `<a href="#fnN" role="doc-noteref">`. The numeric
        // label is recovered from the href and matches the definition collected by
        // processEndnotesSection(), so the pair round-trips to `[^N]` / `[^N]:`.
        // Annotated inline footnotes also carry this role but are reconstructed
        // from their own attribute below, so they are excluded here.
        if (
            $node->getAttribute('role') === 'doc-noteref'
            && !$node->hasAttribute('data-djot-inline-footnote-html')
        ) {
            $ref = ltrim($node->getAttribute('href'), '#');
            if (str_starts_with($ref, 'fn')) {
                return '[^' . substr($ref, 2) . ']';
            }
        }
        ...
        // A non-empty label is already fully escaped: literal `]` from text nodes
        // is neutralized in escapeDjotText(), while structural `]` produced by
        // child elements (a nested image or link) must stay intact. Escaping the
        // whole label here would corrupt that child markup (e.g. an inner
        // `![alt](src)` would become `![alt\](src)`).
    ...
    protected function processList(DOMElement $node): string
    {
        ...
        // A loose list wraps each item's content in a <p>; preserve that by
        // separating items with a blank line so the next parse stays loose
        // rather than collapsing into a tight list.
        $isLoose = $this->isLooseList($node);
        $renderedItems = 0;
    ...
    /**
     * A list is loose when its items carry block-wrapped content: the Djot HTML
     * renderer emits a `<p>` directly inside each `<li>` for loose lists and bare
     * inline content for tight ones. Detecting a single such `<li>` is enough,
     * mirroring the renderer's all-or-nothing tight/loose decision.
     */
    protected function isLooseList(DOMElement $node): bool
    {
        foreach ($node->childNodes as $child) {
            if (!($child instanceof DOMElement) || strtolower($child->tagName) !== 'li') {
                continue;
