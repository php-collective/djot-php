...
DJOT;
    ...
    // ==================== Bracket-in-label fidelity ====================

    public function testImageInsideLinkKeepsStructuralBrackets(): void
    {
        $html = '<p><a href="https://x.com"><img alt="alt" src="/i.png" /></a></p>';

        $this->assertSame("[![alt](/i.png)](https://x.com)\n", $this->converter->convert($html));
    }

    public function testLiteralBracketInLinkTextIsEscaped(): void
    {
        $html = '<p><a href="https://x.com">a]b</a></p>';

        $this->assertSame("[a\\]b](https://x.com)\n", $this->converter->convert($html));
    }

    public function testNestedClosingBracketInLinkLabelIsEscaped(): void
    {
        // The inner emphasis carries a literal `]`; it must be escaped so it does
        // not close the link label early, while the emphasis markers stay intact.
        $html = '<p><a href="https://outer.com"><em>see ]</em></a></p>';

        $this->assertSame("[_see \\]_](https://outer.com)\n", $this->converter->convert($html));
    }

    // ==================== Footnote reference (no annotations) ====================

    public function testFootnoteReferenceWithoutAnnotations(): void
    {
        $html = '<a id="fnref1" href="#fn1" role="doc-noteref"><sup>1</sup></a>';
    ...
    public function testLooseUnorderedListKeepsBlankLines(): void
    {
    ...
    public function testTightListStaysTight(): void
    {
    ...
    public function testExplicitSectionIdIsPreserved(): void
    {
        $html = '<section id="myid"><h1>Heading</h1></section>';

        $this->assertSame("{#myid}\n# Heading\n", $this->converter->convert($html));
    }

    public function testAutoGeneratedSectionIdIsDropped(): void
    {
        $html = '<section id="Heading"><h1>Heading</h1></section>';

        $this->assertSame("# Heading\n", $this->converter->convert($html));
    }

    public function testExplicitSectionIdIsPreservedWhenHeadingHasNoSlug(): void
    {
        // "!!!" normalizes to an empty slug, so Djot would fall back to `s-N`;
        // a non-`s-N` id is therefore explicit and must survive.
        $html = '<section id="custom"><h1>!!!</h1></section>';

        $this->assertSame("{#custom}\n# !!!\n", $this->converter->convert($html));
    }

    public function testFallbackSectionIdIsDropped(): void
    {
        $html = '<section id="s-1"><h1>!!!</h1></section>';
